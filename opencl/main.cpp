...
#define ROWS (64)    // ROWS of vectors a, b, and c
#define COLUMNS (64)
...
int main() {
	...
	cl::Buffer buffer_B(context, CL_MEM_READ_WRITE, sizeof(float) * ROWS * COLUMNS);
	...
	OCL_SAFE_CALL(errcode);
	errcode = queue.enqueueWriteBuffer(buffer_B, CL_TRUE, 0, sizeof(float) * ROWS * COLUMNS, &b[0]);
	OCL_SAFE_CALL(errcode);
	...
	OCL_SAFE_CALL(errcode);
	errcode = kernel.setArg(3, sizeof(int), &N);
	OCL_SAFE_CALL(errcode);
